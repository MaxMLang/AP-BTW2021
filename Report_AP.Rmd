...
# Hohe Werte bestimmen

punkte_spd <- select(filter(btw_trimmed_data, ArbeitslosQ == max(ArbeitslosQ)), Gebiet, ArbeitslosQ)
punkte_spd2 <- select(filter(btw_trimmed_data, SPD.Zweit.End.Perc == max(SPD.Zweit.End.Perc)), Gebiet, SPD.Zweit.End.Perc)
...
punkte_union <- select(filter(btw_trimmed_data, Vfg.Einkommen>30000), Gebiet, Vfg.Einkommen, CDU.Zweit.End.Perc, CSU.Zweit.End.Perc)
...
# Zusammenhang Erst-Zweitstimmen pro Partei
```{r}
...
ggsave("Erst_Zweit_CDU_SPD.png", path = "Grafiken", width = 9, height = 5)
...
ggsave("Erst_Zweit_Rest.png", path = "Grafiken", width = 9, height = 9)
```
...
## Data Preparation
```{r}
# Data Preparation for Briefwahl Analysis
...
btw_trimmed_geo_data %>% 
  ...
  ggtitle("Stimmanteile an Zweitstimmen der Randparteien (AFD und LINKE)")+
