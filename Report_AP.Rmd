...
x <- spd_vs_union %>%
  ...
  xlab(" ") +
  ylab("Schulabgänger/-innen allgemeinblldender Schulen 2019 \nmit allgemeiner und Fachhochschulreife") +
  scale_color_manual(values = c("red", "black")) +
  theme(legend.position = "none")
...
a <- btw_trimmed_data %>%
  replace(is.na(.), 0) %>%
  mutate(Union.Zweit.End.Perc = CDU.Zweit.End.Perc + CSU.Zweit.End.Perc) %>%
  select(c(Union.Zweit.End.Perc,
           SPD.Zweit.End.Perc,
           AFD.Zweit.End.Perc,
           FDP.Zweit.End.Perc,
           ...
           Vfg.Einkommen)) %>%
  pivot_longer(c(Union.Zweit.End.Perc,
           SPD.Zweit.End.Perc,
           AFD.Zweit.End.Perc,
           FDP.Zweit.End.Perc,
           GRÜNE.Zweit.End.Perc,
           LINKE.Zweit.End.Perc), names_to = "Age", values_to = "Percentage") %>%
