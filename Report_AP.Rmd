...
ergebnisse_table[7,3] <- anzahl_stimmen_zweit - sum(ergebnisse_table[1:6,2])

ergebnisse_table[7,4] <- anzahl_stimmen_gesamt - sum(ergebnisse_table[1:6,2])
...
ergebnisse_table %>%
  ...
  xlab("Partei") +
  ylab("Stimmen (in %)") +
  ggtitle(label = "Ergebnis 1. Stimmen Bundestagswahl 2021")
...
btw_kerg_trimmed_bundeslaender %>%
  ggplot(stat = "summary") +
  geom_bar(aes(y = Union.Erst.End.Perc, x = "CDU/CSU"),stat = "summary", fill = "black") +
  geom_bar(aes(y = SPD.Zweit.End.Perc, x = "SPD"),stat = "summary", fill = "red") +
  geom_bar(aes(y = AFD.Zweit.End.Perc, x = "AFD"),stat = "summary", fill = "blue") +
  geom_bar(aes(y = FDP.Zweit.End.Perc, x = "FDP"),stat = "summary", fill = "yellow") +
  geom_bar(aes(y = LINKE.Zweit.End.Perc, x = "Linke"),stat = "summary", fill = "purple") +
  geom_bar(aes(y = GRÜNE.Zweit.End.Perc, x = "Grüne"),stat = "summary", fill = "green") +
  xlab("") +
...
ergebnisse_table %>%
  ggplot(mapping = aes(x = partei, y = stimmen_gesamt_perc)) +
  geom_bar(stat = "identity", fill = partei_colors) +
  xlab("Partei") +
  ylab("Stimmen (in %)") +
  ggtitle(label = "Ergebnis: Gesamtstimmen Bundestagswahl 2021") +
  scale_y_continuous(labels = scales::percent,
                     breaks = scales::pretty_breaks(n = 10))
  
  ggsave("Ergebnis_Gesamtstimmen.png", path="Grafiken/", width = 7, height= 7)
```
...
ggsave("SPD_Union_Zweit_GV_Diff_Box.png", path="Grafiken/", width = 7, height= 7)
```
...
# Briefwahl in Bayern (on Hold, Plausibilitätschecks stimmen nicht)

```{r}
total_brief_urne <- btw_by_brief_urne_wk21 %>% 
  select(B1.Urne, B1Briefwähler) %>% 
  colSums()

names(total_brief_urne) <- c("Urnenwähler", "Briefwähler")
total_brief_urne
sum(total_brief_urne)

btw_by_brief_urne_wk21 %>% 
  select(B) %>% 
  colSums()

btw_by_brief_urne %>% 
  filter((schluessel.Wbz %in% c(9,10)) & (schluessel %in% c(212))) %>% 
  slice(1:2) %>% 
  select(B, B1.Urne, B1Briefwähler, C)
```
