...
btw_trimmed_data %>%
  ggplot(aes(x = ArbeitslosQ, y = SPD.Zweit.End.Perc)) +
  geom_jitter(colour = "red") +
  labs(title = "Arbeitslosenquote") +
  xlab("Arbeitslosenquote Februar 2021") +
  ylab("Zweitstimmen SPD (%)")

ggsave("Scatterplots_ArbeitslosQ_SPD.png", path = "Grafiken/")

a <- select(filter(btw_trimmed_data, ArbeitslosQ == max(ArbeitslosQ)), Gebiet, ArbeitslosQ)
...
btw_trimmed_data %>%
  ...
  ylab("Zweitstimmen Union (%)")

ggsave("Scatterplots_Vermögen_Union.png", path = "Grafiken/")

b <- select(filter(btw_trimmed_data, Vfg.Einkommen>30000), Gebiet, Vfg.Einkommen, CDU.Zweit.End.Perc, CSU.Zweit.End.Perc)
...
btw_trimmed_data %>%
  ...
  ggplot(aes(x = Vfg.Einkommen, y = Percentage, col = Age)) +
  geom_jitter(size = 1) +
  facet_wrap("Age") +
...
ggsave("Scatterplots_Vermögen.png", path = "Grafiken/")
...
is_Osten_data <- btw_trimmed_data %>%
  ...
  select(c(UNION, SPD, GRÜNE, FDP, AFD, LINKE,
           Vfg.Einkommen, f18t34.Perctange, is.Osten)) %>%
  pivot_longer(c(UNION, SPD, GRÜNE, FDP, AFD, LINKE), names_to = "Age", values_to = "Percentage") 

ggplot(is_Osten_data, aes(x = Vfg.Einkommen, y = Percentage, col = Age)) +
  geom_jitter(size = 1, color = "grey") +
  facet_wrap("Age") +
  geom_jitter(data = filter(is_Osten_data, is.Osten == "östliche Bundesländer"), aes(x = Vfg.Einkommen, y = Percentage, col = Age), size = 1) +
  scale_color_manual(values = c("blue", "yellow", "green", "purple", "red", "black")) +
  theme(legend.position = "none") +
  labs(title = "Verfügbares Einkommen", subtitle = "Östliche Bundesländer (farbig)") +
  xlab("Verfügbares Einkommen der privaten Haushalte 2018 \n(EUR je EW)") +
  ylab("Zweitstimmen (%)")

ggsave("Scatterplots_Vermögen_Osten.png", path = "Grafiken/")
...
btw_trimmed_data %>%
  ...
  select(c(UNION, SPD, GRÜNE, FDP, AFD, LINKE,
           f18t34.Perctange)) %>%
  pivot_longer(c(UNION, SPD, GRÜNE, FDP, AFD, LINKE), names_to = "Age", values_to = "Percentage") %>%
  ggplot(aes(x = f18t34.Perctange, y = Percentage, col = Age)) +
  geom_jitter(size = 1) +
  facet_wrap("Age") +
  scale_color_manual(values = c("blue", "yellow", "green", "purple", "red", "black")) +
  theme(legend.position = "none") +
...
ggsave("Scatterplots_Alter_f18t34.png", path = "Grafiken/")

# Junge Wählerschaft Osten

ggplot(is_Osten_data, aes(x = f18t34.Perctange, y = Percentage, col = Age)) +
  geom_jitter(size = 1, color = "grey") +
  facet_wrap("Age") +
  geom_jitter(data = filter(is_Osten_data, is.Osten == "östliche Bundesländer"), aes(x = f18t34.Perctange, y = Percentage, col = Age), size = 1) +
  scale_color_manual(values = c("blue", "yellow", "green", "purple", "red", "black")) +
  theme(legend.position = "none") +
  labs(title = "Alter von 18 bis 34", subtitle = "Östliche Bundesländer (farbig)") +
  xlab("Alter von 18 bis 34 (%) (Stand 31.12.2019)") +
  ylab("Zweitstimmen (%)")

ggsave("Scatterplots_Alter_f18t34_Osten.png", path = "Grafiken/")
...
diff_zweitstimmen <- ergebnisse_table %>%
  ...
  xlab("Partei") +
  ylab("Stimmendifferenz") +
  ggtitle(label = "Differenz an Prozentpunkten der Zweitstimmen (Bundestagswahl 2017 und 2021)") +
  scale_y_continuous(labels = scales::percent,
                     breaks = scales::pretty_breaks(n = 10))
...
ezd_union <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = Union.Erst.End.Perc, y = Union.Zweit.End.Perc), fill = "black", color = "black", shape = 21, alpha = 0.25)+
  scale_x_continuous(limits = c(0, 0.55), position = "bottom")+
  scale_y_continuous(limits = c(0, 0.55))+
  labs(x = NULL, y = "Zweitstimmen in %")
ezd_spd <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = SPD.Erst.End.Perc, y = SPD.Zweit.End.Perc), shape = 21, fill = "red", color = "black", alpha = 0.5)+
  scale_x_continuous(limits = c(0, 0.55))+
  scale_y_continuous(limits = c(0, 0.55))+
  labs(x = NULL, y = NULL)
ezd_gruene <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = GRÜNE.Erst.End.Perc, y = GRÜNE.Zweit.End.Perc), shape = 21, fill = "green", color = "black", alpha = 0.5)+
  scale_x_continuous(limits = c(0, 0.55))+
  scale_y_continuous(limits = c(0, 0.55))+
  labs(x = NULL, y = NULL)
ezd_afd <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = AFD.Erst.End.Perc, y = AFD.Zweit.End.Perc), shape = 21, fill = "blue", color = "black", alpha = 0.5)+
  scale_x_continuous(limits = c(0, 0.4))+
  scale_y_continuous(limits = c(0, 0.4))+
  labs(x = "Erststimmen in %", y = NULL)
ezd_fdp <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = FDP.Erst.End.Perc, y = FDP.Zweit.End.Perc), shape = 21, fill = "yellow", color = "black", alpha = 0.3)+
  scale_x_continuous(limits = c(0, 0.4))+
  scale_y_continuous(limits = c(0, 0.4))+
  labs(x = "", y = "Zweitstimmen in %")
ezd_linke <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = LINKE.Erst.End.Perc, y = LINKE.Zweit.End.Perc), shape = 21, fill = "purple", color = "black", alpha = 0.5)+
  scale_x_continuous(limits = c(0, 0.4))+
  scale_y_continuous(limits = c(0, 0.4))+
  labs(x = "", y = NULL)
ggarrange(ezd_union, ezd_spd, ezd_gruene, ezd_fdp, ezd_afd, ezd_linke, labels = c("UNION", "SPD", "GRÜNE", "FDP", "AFD", "LINKE"), label.y = c(rep(1, 6)), label.x = rep(0.3, 6))
...
data.frame(list(union_diff= union_zweit$DiffProzentPkt,
  ...
  xlab("Union Differenz der Prozenpunkte (Zweitstimmen)")+
  ylab("SPD Differenz der Prozentpunkte (Zweitstimmen)")
...
spd_GV <- btw_trimmed_geo_data %>% 
  ...
  scale_fill_continuous_diverging(name= "Differenz der Prozentpunkte",palette= "Blue-Yellow 2", limit= c(-20,20))+
  ggtitle("Gewinne / Verluste an Prozentpunkten von Zweitstimmen")+
  labs(subtitle = "SPD")+
  xlab("")+
  ylab("")+
  theme_map()
...
total_brief_urne_21_17_df %>% 
  ...
  ggtitle("Vergleich der Anteile von Brief- und Urnenwahl
...
data.frame(list(union_diff= union_erst$DiffProzentPkt,
  ...
  geom_hline(yintercept = 0, linetype= "dashed", colour= "black")+
  geom_vline(xintercept = 0, linetype= "dashed", colour= "black")+
  ggtitle("Vergleich der Gewinne/Verluste an Prozentpunkten der Erststimmen von SPD und Union (bzgl. der Bundestagswahl 2017")+
...
```{r}
btw_trimmed_data %>%
  mutate(is.Osten= as.factor(Bundesland.Nr %in% c(10:16))) %>%
