...
btw_trimmed_data %>%
  ...
  geom_jitter(colour = partei_colors["SPD"]) +
...
btw_trimmed_data %>%
  ...
  geom_jitter(colour = partei_colors["CDU/CSU"]) +
...
ggsave("Scatterplots_Vermögen_Union.png", path = "Grafiken/", width = 9, height = 9)
...
btw_trimmed_data %>%
  ...
  ggplot(aes(x = Vfg.Einkommen, y = Percentage, fill = Age)) +
  geom_jitter(size = 1.75, shape = 21, alpha = 0.4) +
  facet_wrap("Age") +
  scale_fill_manual(values = partei_colors_carla[c(3,4,6,5,2,1)]) +
  theme(legend.position = "none") +
...
ggplot(is_Osten_data, aes(x = Vfg.Einkommen, y = Percentage, col = Age)) +
  ...
  facet_wrap("Age") +
  geom_jitter(data = filter(is_Osten_data, is.Osten == "östliche Bundesländer"), aes(x = Vfg.Einkommen, y = Percentage, col = Age), size = 1.75) +
  scale_color_manual(values =  partei_colors_carla[c(3,4,6,5,2,1)]) +
  theme(legend.position = "none") +
  labs(title = "Verfügbares Einkommen und Zweitstimmen je Partei", subtitle = "Östliche Bundesländer (farbig)") +
  xlab("Verfügbares Einkommen der privaten Haushalte* \n(in Euro je Einwohner)") +
  ylab("Zweitstimmen (in %)") +
  labs(caption = "* Stand: 2018") +
  scale_y_continuous(labels = scales::percent)
...
btw_trimmed_data %>%
  ...
  select(c(UNION, SPD, GRÜNE, FDP, AFD, LINKE,
           f18t34.Perctange)) %>%
  pivot_longer(c(UNION, SPD, GRÜNE, FDP, AFD, LINKE), names_to = "Age", values_to = "Percentage") %>%
  ggplot(aes(x = f18t34.Perctange, y = Percentage, fill = Age)) +
  geom_jitter(size = 1.75, shape = 21, alpha = 0.4) +
  facet_wrap("Age") +
  scale_fill_manual(values = partei_colors_carla[c(3,4,6,5,2,1)]) +
  theme(legend.position = "none") +
...
ggplot(is_Osten_data, aes(x = f18t34.Perctange, y = Percentage, col = Age)) +
  geom_jitter(size = 1.75, color = "grey") +
  facet_wrap("Age") +
  geom_jitter(data = filter(is_Osten_data, is.Osten == "östliche Bundesländer"), aes(x = f18t34.Perctange, y = Percentage, col = Age), size = 1.75) +
  scale_color_manual(values = partei_colors_carla[c(3,4,6,5,2,1)]) +
  theme(legend.position = "none") +
  labs(title = "Anteil der 18- bis 34-Jährigen und Zweitstimmen je Partei", subtitle = "Östliche Bundesländer (farbig)") +
  xlab("Anteil der 18- bis 34-Jährigen an der Gesamtbevölkerung* (in %)") +
  ylab("Zweitstimmen (in %)") +
  scale_y_continuous(labels = scales::percent) +
  labs(caption = "* Stand 31.12.2019")
...
ezd_union <- btw_trimmed_data %>%
  ggplot()+
  geom_point(aes(x = Union.Erst.End.Perc, y = Union.Zweit.End.Perc), fill = partei_colors["CSU/CDU"], color = "black", shape = 21, alpha = 0.5)+
  scale_x_continuous(labels = scales::percent,
...
union_GV <- btw_trimmed_geo_data %>% 
  ...
  geom_sf(data = länder, color = "black", fill = NA)+
  scale_fill_continuous_diverging(labels = scales::percent,
                                  name= "Differenz der Zweitstimmen (in Prozentpunkten)",
                                  palette= "Blue-Yellow 2", limit= c(-0.2,0.2))+
  ggtitle("Union")+
  xlab("")+
  ylab("")+
...
MittSA <- FW_Diff_Data %>% 
  ...
  labs(colour= "Regierungsbeteiligung (in %)") +
  scale_y_continuous(labels = scales::percent) + 
  scale_x_continuous(labels = scales::percent)
...
p1 <- ggarrange(HW, MittSA,SozPfli.LandW, common.legend = TRUE, legend = "top", nrow= 1)
annotate_figure(p1, top = text_grob("Beispiele positiver Korrelation: Anteil an Zweitstimmen und ausgewählte Strukturvariablen", color = "black", size = 15))
ggsave("LT_FW_ZweitPerc_Scatter_PosKor.png", path = "Grafiken/", width = 15, height= 7)


p1 <- ggarrange(Boden.SieduVerk, Schulab.mitAllgHS, SozPfli.ÖffPrivDienstl, nrow = 1,common.legend = TRUE, legend = "top")
annotate_figure(p1, top = text_grob("Beispiele negativer Korrelation: Anteil an Zweitstimmen und ausgewählte Strukturvariablen", color = "black", size = 15))
...
ggsave("Union_Zweit_Map.png", path = "Grafiken/", width= 7, height= 7)
```
## SPD
```{r}
btw_trimmed_geo_data %>%
...
ggsave("SPD_Zweit_Map.png", path = "Grafiken/", width= 7, height= 7)
```
## AFD
```{r}
btw_trimmed_geo_data %>%
...
ggsave("AFD_Zweit_Map.png", path = "Grafiken/", width= 7, height= 7)
```
## FDP
```{r}
btw_trimmed_geo_data %>%
...
ggsave("FDP_Zweit_Map.png", path = "Grafiken/", width= 7, height= 7)
```
## Grüne
```{r}
btw_trimmed_geo_data %>%
...
ggsave("Gruene_Zweit_Map.png", path = "Grafiken/", width= 7, height= 7)
```
...
ggsave("Linke_Zweit_Map.png", path = "Grafiken/", width= 7, height= 7)
```
